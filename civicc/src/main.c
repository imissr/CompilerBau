...
static void Usage(char *program)
{
  ...
  printf("  --max-factor/-m <number>     Enable super strength reduction with max factor.\n");
}
...
static int ProcessArgs(int argc, char *argv[])
{
  static struct option long_options[] = {
      ...
      {"max-factor", required_argument, 0, 'm'},
      {0, 0, 0, 0}};
  ...
  while (1)
  {
    c = getopt_long(argc, argv, "hsvo:b:m:", long_options, &option_index);
    ...
    case 'o':
      ...
      break;
    case 'm':
      global.max_strength_reduction_factor = (int)strtol(optarg, NULL, 10);
      break;
    case 'h':
      Usage(argv[0]);
      exit(EXIT_SUCCESS);
